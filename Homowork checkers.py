...
class Warcaby:
    # polem klasowym jest słownik zawierający osiem słowników
    warcaby = {
...
#       print(self.warcaby[3][3])
    def checkMoveFromTo(self,rowStart,columnStart,rowStop,columnStop, type):
        #sprawdzam czy punkty znajdujs sie w lkuczach naszych słowników
        if(rowStart in self.warcaby.keys() and columnStart in self.warcaby[rowStart].keys()
            ...
            #sprawdzenie poprawności ruchu dla pionka X
            if (type=="X"and rowStop ==(rowStart+1)
                ...
                #sprawdzenie poprawności ruchu pionka 0
    ...
    def movePoint(self,rowStart,columnStart,rowStop,columntStop,type):
        #przesuniecie pionk na nową pozycje
        self.warcaby[rowStop][columntStop]=type
        #aktualizacja pustego miejsca pozostałegopo pionku
        self.warcaby[rowStart][columnStart]="_"
        self.printBoard()


w1 = Warcaby()
w1.printBoard() #na obiekcie klasy Warcaby (w1) uruchamiamy metodę
#w1.getPoint()
